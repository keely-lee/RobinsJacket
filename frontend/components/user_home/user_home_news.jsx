    ...
    if (this.props.news.data) {
      ...
      return news.map((article, _idx) => {
        ...
        return (
          ...
          >
            <a
              href={content.clickThroughUrl?.url}
              className={`news-a-${id}`}
              key={`a-${id}`}
            >
              <div className="user-inner-news-div" key={`div2-${id}`}>
                <section className="user-news-left" key={`sec1-${id}`}>
                  <h6 className={`news-date-${id}`} key={`date-${id}`}>
                    {new Date(content.pubDate).toDateString()}
                  </h6>
                  <h6 className={`source-name-${id}`} key={`name-${id}`}>
                    {content.provider.displayName}
                  </h6>
                  <h5 className={`news-title-${id}`} key={`title-${id}`}>
                    {content.title}
                  </h5>
                ...
                {content.thumbnail ? (
                  <section className="user-news-right" key={`sec2-${id}`}>
                    <img
                      src={content.thumbnail.resolutions[0].url}
                      className={`news-pic-${id}`}
                      key={`src-${id}`}
                      draggable="false"
                    />
                  </section>
